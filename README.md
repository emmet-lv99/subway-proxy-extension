...
### 2. 서울시 API 부하 방지를 위한 Supabase 프록시 캐시 서버 구축
...
유저의 시간 조건 충족 여부에 따라 15초 추적 타이머를 온디맨드 형태로 스케줄링 제어합니다.
...
* **캐시 만료 검증:** 최종 동기화 후 15초가 지나지 않았다면 서울시 API 호출을 생략하고 DB 스냅샷을 반환하여 인프라를 보호합니다.
