...
* **해결 방안:** `chrome.alarms` API를 연동하여 브라우저 엔진에 지속적으로 시그널을 전달함으로써 서비스 워커가 종료되지 않도록 **백그라운드 영속성(Persistence)을 안정적으로 확보**했습니다.
* **리소스 최적화:** 모니터링이 필요 없는 주말이나 심야 시간대까지 무식하게 타이머를 구동하는 것은 클라이언트 리소스와 서버 트래픽 측면에서 대단히 비효율적입니다. 따라서 유저가 설정한 특정 요일과 출퇴근 시간대를 판별하는 **[Time-Window 정밀 스케줄러]**를 구현했습니다. 지정된 시간 범위 내에서만 15초 주기 초정밀 타이머를 가동하고, 범위 밖으로 벗어나면 타이머 인스턴스를 즉시 파괴(Destroy)하여 자원 낭비를 완벽히 차단했습니다.
